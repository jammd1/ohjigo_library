  ...
  return (
    <main className="main-content about-page">
      ...
      <div className="tech-section">
        ...
        <br />
        <h3>도입 배경</h3>
        <p>기존에는 도서 목록 구축 과정에서 도서 분야를 사람이 직접 판단하여 수기로 입력해야했다.
          이는 잦은 오류와 긴 소요시간 등의 문제를 야기했다. 
          해당 문제를 해결하고자 AI 모델을 학습시켜
          도서를 입력하는 사서들의 업무 난이도를 줄이고자 하였다.</p>
          <br />
        <h3>모델 학습</h3>
        <p>Berlin State Library 제공 다국어 도서 공개 데이터셋 28525개를 
          8:1:1 비율로 나눠 각각 training, evaluation, test로 사용하였다.
          문학, 어학, 사회과학, 역사 class의 multiclass classification을 목적으로 Task-specific Fine-tuning을 진행하였고,
          우리 도서관이 독일어도서 뿐만 아니라 한국어, 영어, 라틴어 등 다국어 도서를 포함하고 있는 점을 고려하여
          pretrained model로 mBERT를 사용하였다.
          추가로 학습 데이터 class의 불균형 (
          역사 33.24%, 문학31.51%, 사회과학26.95%, 어학8.29%) 보완을 위해 어학 class에 가중치를 부여하였다.</p>
          <br />
          <p>주요 성능 지표는 다음과 같다:</p>
          ...
          <br />
          역사 부문에서 모델의 오분류 정도가 큰 점을 고려, 분류 작업의 정확도 향상을 위하여 실제 inference에서는 
          confidence에 따른 오름차순 class 목록이 출력되어 사람이 직접 선택할 수 있는 구조이다.
          <br />
          저장된 가중치는 다음 링크에서 다운로드 받을 수 있다: jsjang0104/book-genre-classifier-bert
        <br />
        <br />
        <br />
        ...
        <p>도서 목록 구축 기존 과정에서는 분야와 마찬가지로 청구 기호 또한 사람이 직접 만들어내는 구조로서 긴 소요시간과 더불어 
          작업 난이도로 인해 긴 청구 기호 생성이 불가능하였다. 또한 만들어진 청구기호가 중복되는 경우도 많아 도서를 고유하게 분류하는 용도의 청구기호로서의 의의가 퇴색되었었다.
          해당 문제를 해결하고자 중복없이 도서의 모든 정보를 담아내는 문자열을 생성하는 도구를 개발하여 
          청구기호의 본질적 의의를 되찾고 도서 입력 업무 시간을 획기적으로 줄이고자 하였다.</p>
        <br />
        <h3>작동 원리</h3>
        <p>사용 언어: Python</p>
        ...
        <br />
        <p>해당 파이썬 파일은 다음 링크에서 다운로드 받을 수 있다: https://drive.google.com/file/d/1htjFnBmoWcSdOwK-XJWu4HcRhK68ZqGH/view?usp=drive_link</p>
        <br />
        <br />
